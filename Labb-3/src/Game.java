...
public class Game {
	private Ball ball;
	private Bat bat;
	private RectGrid rgrid;
	// private SquareCollection sqcoll;
	private ColoredBox cBox;
	...
	public Game(GameBoard board) {
		...
		cBox = new ColoredBox(650, 90, 20, 20, Color.blue);
		rgrid = new RectGrid(3, 7);
		Text.life = 3;
		HighScore.HScore();
		// sqcoll = new SquareCollection();
	}

	/*
	 * boolean hit_left = Collision.Left(ball.getX(), ball.getY(), ball.getWidth(),
	 * ball.getHeight(), bat.getX(), bat.getY(), bat.getHeight(), bat.getHeight());
	 * boolean hit_right = Collision.Right(ball.getX(), ball.getY(),
	 * ball.getWidth(), ball.getHeight(), bat.getX(), bat.getY(), bat.getHeight(),
	 * bat.getHeight()); boolean hit_top = Collision.Top(ball.getX(), ball.getY(),
	 * ball.getWidth(), ball.getHeight(), bat.getX(), bat.getY(), bat.getHeight(),
	 * bat.getHeight());
	 */
	...
	public void update(Keyboard keyboard) {
		if (Text.life >= 1 && totalBricks > 0) {
			...
			if (gamePaused == true) {
				...
				bat.update(keyboard); // this moves your ball, players, etc
				// sqcoll.update(keyboard);
				// cBox.update(keyboard);
